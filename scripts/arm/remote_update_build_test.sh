...
if [[ "$AUTOJIT_USE_JITLIST_FILTER" == "1" ]]; then
  ...
  env PYTHONJITAUTO="1000000" CINDERX_WORKER_PYTHONJITAUTO="$AUTOJIT_GATE" PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" PYTHONJITDEBUG=1 PYTHONJITLOGFILE="$LOG" \
    CINDERX_JITLIST_ENTRIES="$AUTOJIT_JITLIST_ENTRIES" PYTHONJITENABLEJITLISTWILDCARDS=1 \
    python -m pyperformance run --debug-single-value -b "$BENCH" \
      --inherit-environ PYTHONPATH,CINDERX_WORKER_PYTHONJITAUTO,PYTHONJITAUTO,PYTHONJITDEBUG,PYTHONJITLOGFILE,CINDERX_JITLIST_ENTRIES,PYTHONJITENABLEJITLISTWILDCARDS,CINDERX_ENABLE_SPECIALIZED_OPCODES \
      -o "/root/work/arm-sync/${BENCH}_autojit${AUTOJIT_GATE}_${RUN_ID}.json"
else
  env PYTHONJITAUTO="1000000" CINDERX_WORKER_PYTHONJITAUTO="$AUTOJIT_GATE" PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" PYTHONJITDEBUG=1 PYTHONJITLOGFILE="$LOG" \
    python -m pyperformance run --debug-single-value -b "$BENCH" \
      --inherit-environ PYTHONPATH,CINDERX_WORKER_PYTHONJITAUTO,PYTHONJITAUTO,PYTHONJITDEBUG,PYTHONJITLOGFILE,CINDERX_ENABLE_SPECIALIZED_OPCODES \
      -o "/root/work/arm-sync/${BENCH}_autojit${AUTOJIT_GATE}_${RUN_ID}.json"
fi
deactivate
