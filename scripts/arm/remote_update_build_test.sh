...
CINDERX_PYPERF_PRECOMPILE_ALL="${CINDERX_PYPERF_PRECOMPILE_ALL:-0}"
...
if [[ "$CINDERX_PYPERF_PRECOMPILE_ALL" != "0" && "$CINDERX_PYPERF_PRECOMPILE_ALL" != "1" ]]; then
  echo "ERROR: CINDERX_PYPERF_PRECOMPILE_ALL must be 0 or 1, got '$CINDERX_PYPERF_PRECOMPILE_ALL'"
  exit 1
fi
...
env CINDERX_WORKER_PYTHONJITAUTO="$SMOKE_AUTOJIT" \
  ...
  CINDERX_PYPERF_PRECOMPILE_ALL="$CINDERX_PYPERF_PRECOMPILE_ALL" \
  ...
  "$DRIVER_VENV/bin/python" scripts/arm/verify_pyperf_venv.py \
    ...
    --worker-env=CINDERX_PYPERF_WORKER=1 \
    --worker-env=CINDERX_PYPERF_PRECOMPILE_ALL="$CINDERX_PYPERF_PRECOMPILE_ALL" \
...
env PYTHONJITAUTO="$SMOKE_AUTOJIT" CINDERX_PYPERF_WORKER=1 CINDERX_PYPERF_PRECOMPILE_ALL="$CINDERX_PYPERF_PRECOMPILE_ALL" PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" CINDERX_ENABLE_SPECIALIZED_OPCODES="$CINDERX_ENABLE_SPECIALIZED_OPCODES" "$PYVENV_PATH/bin/python" -c 'g=(i for i in [1]); next(g, None); import re; re.compile("a+"); print("smoke-ok")'
...
env PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" CINDERX_PYPERF_WORKER=1 CINDERX_PYPERF_PRECOMPILE_ALL="$CINDERX_PYPERF_PRECOMPILE_ALL" CINDERX_JITLIST_ENTRIES="$JITLIST_ENTRIES" PYTHONJITENABLEJITLISTWILDCARDS=1 \
  python -m pyperformance run --debug-single-value -b "$BENCH" \
    --inherit-environ PYTHONPATH,CINDERX_PYPERF_WORKER,CINDERX_PYPERF_PRECOMPILE_ALL,CINDERX_JITLIST_ENTRIES,PYTHONJITENABLEJITLISTWILDCARDS,CINDERX_ENABLE_SPECIALIZED_OPCODES \
...
if [[ "$AUTOJIT_USE_JITLIST_FILTER" == "1" ]]; then
  ...
  env PYTHONJITDISABLE=1 CINDERX_WORKER_PYTHONJITAUTO="$AUTOJIT_GATE" CINDERX_PYPERF_WORKER=1 CINDERX_PYPERF_PRECOMPILE_ALL="$CINDERX_PYPERF_PRECOMPILE_ALL" PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" PYTHONJITDEBUG=1 PYTHONJITLOGFILE="$LOG" \
    CINDERX_JITLIST_ENTRIES="$AUTOJIT_JITLIST_ENTRIES" PYTHONJITENABLEJITLISTWILDCARDS=1 \
    python -m pyperformance run --debug-single-value -b "$BENCH" \
      --inherit-environ PYTHONPATH,CINDERX_WORKER_PYTHONJITAUTO,CINDERX_PYPERF_WORKER,CINDERX_PYPERF_PRECOMPILE_ALL,PYTHONJITDEBUG,PYTHONJITLOGFILE,CINDERX_JITLIST_ENTRIES,PYTHONJITENABLEJITLISTWILDCARDS,CINDERX_ENABLE_SPECIALIZED_OPCODES \
      -o "/root/work/arm-sync/${BENCH}_autojit${AUTOJIT_GATE}_${RUN_ID}.json"
else
  env PYTHONJITDISABLE=1 CINDERX_WORKER_PYTHONJITAUTO="$AUTOJIT_GATE" CINDERX_PYPERF_WORKER=1 CINDERX_PYPERF_PRECOMPILE_ALL="$CINDERX_PYPERF_PRECOMPILE_ALL" PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" PYTHONJITDEBUG=1 PYTHONJITLOGFILE="$LOG" \
    python -m pyperformance run --debug-single-value -b "$BENCH" \
      --inherit-environ PYTHONPATH,CINDERX_WORKER_PYTHONJITAUTO,CINDERX_PYPERF_WORKER,CINDERX_PYPERF_PRECOMPILE_ALL,PYTHONJITDEBUG,PYTHONJITLOGFILE,CINDERX_ENABLE_SPECIALIZED_OPCODES \
      -o "/root/work/arm-sync/${BENCH}_autojit${AUTOJIT_GATE}_${RUN_ID}.json"
fi
deactivate
