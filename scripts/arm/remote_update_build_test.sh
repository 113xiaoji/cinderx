...
if [[ "$AUTOJIT_USE_JITLIST_FILTER" == "1" ]]; then
  ...
  env PYTHONJITAUTO="1000000" CINDERX_WORKER_PYTHONJITAUTO="$AUTOJIT_GATE" PYTHONPATH="$HOOK_DIR${PYTHONPATH:+:$PYTHONPATH}" PYTHONJITDEBUG=1 PYTHONJITLOGFILE="$LOG" CINDERX_DEFER_WORKER_JIT=1 \
