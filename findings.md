...
- Extended the existing recipe-driven configuration layer in
  [bench_pyperf_direct.py](C:/work/code/cinderx1/cinderx/scripts/arm/bench_pyperf_direct.py):
  - `--recipe-json`
  - recipes can now also carry benchmark identity:
    - `benchmark_name`
    - `module_name`
    - `bench_func`
    - `bench_args`
- Added the first checked-in self-contained recipe:
...
- Remote verification:
  - deployed via the standard ARM archive + `remote_update_build_test.sh` path
    on `/root/venv-cinderx314-recipes`
  ...
  - remote script tests on the deployed tree:
    - `Ran 11 tests ... OK`
  - self-contained recipe smoke:
    - `python scripts/arm/bench_pyperf_direct.py --recipe-json scripts/arm/reprofile_recipes/bm_go_board_useful.json --samples 1`
    - `reprofiled_count = 1`
