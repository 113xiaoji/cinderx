...
class ArmRuntimeTests(unittest.TestCase):
    ...
    def test_phase1_loop_osr_skips_high_call_wrapper_shape(self) -> None:
        code = textwrap.dedent(
            """
            import cinderx.jit as jit
        ...
        with tempfile.TemporaryDirectory() as tmp:
            script = f"{tmp}/phase1_high_call_wrapper_shape.py"
            with open(script, "w", encoding="utf-8") as fp:
                fp.write(code)
            ...
            lines = [line.strip() for line in proc.stdout.splitlines() if line.strip()]
            self.assertGreaterEqual(len(lines), 3, proc.stdout)
            self.assertEqual(int(lines[-3]), 99920000, proc.stdout)
            self.assertEqual(int(lines[-2]), 0, proc.stdout)
            self.assertEqual(lines[-1], "False", proc.stdout)

    def test_phase1_loop_osr_reports_skip_reason_for_high_call_wrapper(self) -> None:
        code = textwrap.dedent(
            """
            import cinderx.jit as jit
            ...
            skip_entries = [
                ...
                if entry["normal"]["func_qualname"] == "hot"
                and entry["normal"]["reason"] == "high_call_wrapper"
            ]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            script = f"{tmp}/phase1_high_call_wrapper_skip_reason.py"
            with open(script, "w", encoding="utf-8") as fp:
                fp.write(code)
    ...
    def test_recursive_coroutine_hir_inliner_force_compile_does_not_crash(
        self,
    ) -> None:
        # Regression guard:
        # HIR inliner compilation of recursive coroutines should not crash when
        # Send type inference encounters an empty assign chain. The fallback
        # behavior should stay conservative and still produce a working compile.
        code = textwrap.dedent(
            """
            import cinderx.jit as jit
            import cinderjit
        ...
        with tempfile.TemporaryDirectory() as tmp:
            script = f"{tmp}/recursive_coroutine_hir_inliner_force_compile.py"
            with open(script, "w", encoding="utf-8") as fp:
                fp.write(code)
